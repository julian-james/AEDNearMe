...
class TestViewsCase(TestCase):
    ...
    def test_register(self):
        response = self.c.post(reverse('users/register'), {'username': 'test', 'email': 'test@test.com', 'password':'test'})
        self.assertEqual(response.status_code, 201)
    
    def test_login(self):
        self.c.post(reverse('users/register'), {'username': 'test', 'email': 'test@test.com', 'password':'test'})
    ...
    def test_logout(self):
        ...
        print(token)
        response = self.c.post(reverse('users/logout'), **{'Authorization': f'Token {token}'})
        self.assertEqual(response.status_code, 200)
